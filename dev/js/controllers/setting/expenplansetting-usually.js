define(['../../app', '../../common/utilities'], function(app, UTILITIES) {
	...
	return ['$scope', '$http', '$mdDialog', '$mdToast', 'CONSTANT', function($scope, $http, $mdDialog, $mdToast, CONSTANT) {
		...
		// 加载平日计费方案
		$scope.loadSimpleList = function() {
			$http.post(CONSTANT.SERVICE.SETTING.EXPENPLANSETTING.LIST_SIMPLE + '?shopId=' + $scope.userInfo.shopId).success(function(data) {
				$scope.simpleList = data.data;
				for (var h = 0; h < data.data.length; h++) {
					for (var i = 0; i < data.data[h].timepart.length; i++) {
						for (var j = 0; j < data.data[h].timepart[i].tableType.length; j++) {
							var tablePrice = data.data[h].timepart[i].tableType[j].tablePrice;
			...
			// $http({
			// 	method: 'POST',
			// 	url: CONSTANT.SERVICE.SETTING.EXPENPLANSETTING.LIST_SIMPLE,
			// 	data: {
			...
			$scope.getBoxTypeList(function() {
				if (!$scope.statuStore.expenplanTypeIndex) {
					...
					$mdDialog.show({
						...
						templateUrl: 'tmpl/setting/addExpenplanUsually.tmpl.html',
		...
		var editController = function($scope, $rootScope, $mdDialog, $mdToast, CONSTANT, parentScope, boxTypeList, modalCache) {
			...
			$scope.save = function() {
				...
				// $http.post(CONSTANT.SERVICE.SETTING.EXPENPLANSETTING.SAVE_SIMPLE + '?actCreateStr=' + reqStr).success(function(data) {
				// 	if (data.code === 0) {
				...
				$http({
					method: 'POST',
					url: CONSTANT.SERVICE.SETTING.EXPENPLANSETTING.SAVE_SIMPLE,
					data: angular.fromJson(req)
				}).success(function(data) {
					if (data.code === 0) {
						parentScope.showSuccess();
					} else {
		...
		$scope.deleteItem = function(ev, id) {
			...
			$mdDialog.show(confirm).then(function() {
				$http.post(CONSTANT.SERVICE.SETTING.EXPENPLANSETTING.DELETE + '?activeId=' + id + '&shopId=' + $scope.userInfo.shopId).success(function(data) {
		...
		$scope.focusPrice = function(e, obj, i) {
		...
		// 保存单价
		$scope.savePrice = function(priceId, e, obj, i) {
			if (!priceId) {
				$mdToast.show(
					$mdToast.simple()
